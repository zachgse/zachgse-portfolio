...
export async function generateMetadata(
    ...
    const format = slug.charAt(0).toUpperCase() + slug.slice(1);
    return {
...
const ProjectSlug = async({ params }: { params: Promise<{ slug: string }> }) => {
    const {slug} = await params;
    const projects = await cachedFetch(`projects?slug=eq.${slug}&limit=1`);
